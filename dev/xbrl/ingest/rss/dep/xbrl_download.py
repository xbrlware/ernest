...
def SECdownload(year, month):
    ...
    try:
        ...
        print('TimeoutError')
    if not good_read:
        print("Unable to download RSS feed document:", year, month)
        return
    # we have to unfortunately use both feedparser (for normal cases)
    # and ET for old-style RSS feeds, because feedparser cannot handle the
    # case where multiple xbrlFiles are referenced without enclosure
    try:
    ...
    for item in feed.entries:
        ...
        try:
            ...
            if (len(enclosures) > 0):
                # ZIP file enclosure exists, we can just download the ZIP file
                ...
                # download all XBRL files here and ZIP them ourselves
                ...
                if os.path.isfile(zipfname):
                    ...
                    xbrlFiling = currentItem.find(
                        "edgar:xbrlFiling", edgarNamespace)
                    xbrlFilesItem = xbrlFiling.find(
                        "edgar:xbrlFiles", edgarNamespace)
                    xbrlFiles = xbrlFilesItem.findall(
                        "edgar:xbrlFile", edgarNamespace)
                    ...
                    try:
                        for xf in xbrlFiles:
                            ...
                            if xfurl.endswith((".xml", ".xsd")):
                                targetfname = \
                                    target_dir + "temp/" + xfurl.split('/')[-1]
                                retry_counter = 3
                                while retry_counter > 0:
                                    good_read = \
                                        downloadfile(xfurl, targetfname)
                                    if good_read:
                                        break
                                    else:
                                        print("Retrying:", retry_counter)
